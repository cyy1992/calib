...
typedef struct DmtxRegion_struct {
   ...
   double		   res_angle;		//下边缘与x轴夹角精确角度（弧度，-pi~pi）
   DmtxVector2	   res_loc;			//角点精确坐标
} DmtxRegion;
...
{
	uint16_t arriveX;			//最高位为assigned，1到15位为正方向的x坐标，最高只能保存32767以内的坐标值
	uint16_t arriveY;			//1到15位为正方向的y坐标
	uint16_t arriveX_;			//最高位为assigned，1到15位为负方向的x坐标
	uint16_t arriveY_;			//1到15位为负方向的y坐标
...
/* int             cacheComplete; */
   ...
   int			   solidContiAngRng;
   int             dashedContiAngRng;
...
extern DmtxDecode* dmtxDecodeCreate(DmtxImage *img, int16_t lastBottomAngle, int edgeMin, int edgeMax, int edgeThresh, int edgeStopThresh, int solidContiAngRng, int dashedContiAngRng, int dmType, int coarse2fineMode, int outlineLen, DmtxImgInfo* acheBuf, DmtxPixelLoc* outlineBuf);
...
extern DmtxPassFail dmtxDecodeMatrixRegion(DmtxDecode *dec, DmtxRegion *reg, DmtxMessage *msg);

/* dmtxregion.c */
extern DmtxPassFail dmLocCenter(const uint8_t* srcImg, DmtxVector2* centerLoc, const int imageWidth, const int imageHeight, const int leastGridCnt, const int gridSize, const int locDriftThresh, const float ratio_k, const int min_min_k, const int min_max_k, const int max_max_k);
extern uint16_t calc_ratio(const uint8_t* cur_point, const int width, const int height, const int widthStep, const int index, uint8_t* loc_map, const float ratio_k, const int min_min_k, const int min_max_k, const int max_max_k);

extern int dmtxRegionFindNext(DmtxDecode *dec, DmtxRegion *reg);
...
/* dmtxmessage.c */
extern void dmtxMessageCreate(int sizeIdx, int symbolFormat, DmtxMessage* message);
